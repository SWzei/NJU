...
git clone git@github.com:SWzei/NJU.git /var/www/linquan/repo
...
- keep `Linquan/linquan-website/.gitignore` with the project so upload/runtime ignore rules stay with the website subtree
...
git switch -c feat/linquan-change
...
### 12.5 Branch + PR quick reference

Use a feature branch and stage only the intended website paths:
...
git switch -c feat/linquan-change
git status --short
git add -- .gitignore Linquan/linquan-website
git diff --cached --stat
git commit -m "your message"
git push -u origin feat/linquan-change
```

Then open a pull request into `main`. Do not push directly to `main` and do not force-push shared branches.
